...
int fs_mount_all(void)
{
    ...
    // Try to create OPFS backend for /home (persistent storage)
    // This may fail on main thread without JSPI, which is OK
    printf("Attempting to create OPFS backend...\n");

    // For now, skip OPFS creation to avoid main thread issues
    // TODO: Implement OPFS in a worker thread
    printf("⚠ Skipping OPFS backend creation (main thread limitation)\n");
    printf("Using MemFS for all mount points\n");

    // Create necessary directories
    mkdir("/home/.tmp", 0755);

    printf("Filesystems ready: /home (MemFS), /tmp (MemFS)\n");
    return 0;
}
