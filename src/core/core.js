...
if (!ENVIRONMENT_IS_PTHREAD) {
  if (Module["wasmMemory"]) {
    ...
    var INITIAL_MEMORY = Module["INITIAL_MEMORY"] || 1048576;
...
function __wasmfs_opfs_read_access(accessID, bufPtr, len, pos_low, pos_high) {
  var pos = convertI32PairToI53Checked(pos_low, pos_high);
  let accessHandle = wasmfsOPFSAccessHandles.get(accessID);
...
async function __wasmfs_opfs_read_blob(ctx, blobID, bufPtr, len, pos_low, pos_high, nreadPtr) {
  var pos = convertI32PairToI53Checked(pos_low, pos_high);
...
function __wasmfs_opfs_write_access(accessID, bufPtr, len, pos_low, pos_high) {
  var pos = convertI32PairToI53Checked(pos_low, pos_high);
  let accessHandle = wasmfsOPFSAccessHandles.get(accessID);
  let data = GROWABLE_HEAP_U8().subarray(bufPtr, bufPtr + len);
  try {
...
var proxiedFunctionTable = [ _proc_exit, exitOnMainThread, pthreadCreateProxied, __setitimer_js ];
