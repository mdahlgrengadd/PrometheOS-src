...
const ActionParameterInput: React.FC<{
  ...
  const convertValue = (value: string): string | number | boolean => {
    if (
      "enum" in parameter &&
      Array.isArray((parameter as { enum: string[] }).enum)
    ) {
      return value;
    }
    ...
    <div className="mb-4">
      ...
      Array.isArray((parameter as { enum: string[] }).enum) ? (
        ...
        >
          ...
          {(parameter as { enum: string[] }).enum.map((option: string) => (
  ...
  return (
    <div className="h-full flex flex-col bg-background text-foreground p-4">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold">Custom API Explorer</h1>
