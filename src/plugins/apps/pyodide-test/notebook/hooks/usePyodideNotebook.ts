...
export const usePyodideNotebook = (
  initialCells?: UsePyodideNotebookProps["initialCells"]
) => {
  const generateId = () =>
      ...
      try {
        ...
        if (!isInitialized) {
          ...
          if (initResult.status === "success") {
            ...
            throw new Error(
              `Pyodide initialization failed: ${initResult.message}`
            );
          }
        }

        // Execute the Python code
        const result = await workerPluginManager.executePython(
          cell.content,
          true
        );
