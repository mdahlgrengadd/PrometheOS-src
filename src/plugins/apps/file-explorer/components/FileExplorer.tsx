...
import { FileSystemItem, User, ContextMenuPosition } from '../types/fileSystem';
...
const FileExplorer: React.FC = () => {
  ...
  const [expandedFolders, setExpandedFolders] = useState<Set<string>>(new Set(['root', 'desktop', 'documents']));
  ...
  const [isDraggingOnEmptySpace, setIsDraggingOnEmptySpace] = useState<boolean>(false);
  ...
  const findItemByPath = useCallback((path: string[]): FileSystemItem | null => {
  ...
  // Get current folder
  const getCurrentFolder = useCallback(() => findItemByPath(currentPath), [findItemByPath, currentPath]);
  ...
  const navigateToPathLevel = useCallback((level: number) => {
  ...
  function handleSelectionChange(selectionBox: {top: number, left: number, width: number, height: number}) {
  ...
  const deleteItem = (itemId: string) => {
    ...
    setFileSystem(prev => {
      ...
      const target = findItemByPath(currentPath);
      if (target && target.children) {
        target.children = target.children.filter(child => child.id !== itemId);
      }
      return updated;
    });
    setSelectedItems(new Set());
  ...
  const handleFolderDrop = (targetFolderId: string, e: React.DragEvent) => {
    ...
    let targetFolderPath = findFolderPath(fileSystem, targetFolderId);
    ...
    setFileSystem(prev => {
      ...
      const movedItems = sourceFolder.children.filter(item => itemsToMove.includes(item.id));
      sourceFolder.children = sourceFolder.children.filter(item => !itemsToMove.includes(item.id));
  ...
  const currentFolder = getCurrentFolder();
  const breadcrumbs = currentPath.slice(1).reduce((acc: Array<{id: string, name: string, path: string[]}>, pathId, index) => {
    ...
    <div className="h-screen flex flex-col bg-white" onClick={handleClickOutside}>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        >
          ...
          {currentFolder && (
            <>
              <div className="mb-6">
                <h2 className="text-xl font-semibold text-gray-800 mb-1">{currentFolder.name}</h2>
                <p className="text-sm text-gray-500">
                  {currentFolder.children?.length || 0} item{(currentFolder.children?.length || 0) !== 1 ? 's' : ''}
                </p>
              </div>
      ...
      {showContextMenu && (
        <ContextMenu 
          ...
          onRename={(id) => {
            const currentItem = currentFolder?.children?.find(item => item.id === id);
            if (currentItem) {
