...
const ThemeSelector: React.FC = () => {
  ...
  const [showWindowsDemo, setShowWindowsDemo] = useState(false);
  ...
  // Check if current theme is a Windows theme
  ...
  return (
    <>
      ...
      >
        <div className="flex w-full items-center justify-between">
          ...
          <div className="flex gap-2">
            {isWindowsTheme && (
              <button
                onClick={() => setShowWindowsDemo(true)}
      ...
      {showWindowsDemo && (
        <WindowsDemo onClose={() => setShowWindowsDemo(false)} />
      )}
    </>
  );
};
