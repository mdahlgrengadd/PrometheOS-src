...
export const UnifiedWindowShell: React.FC<WindowShellProps> = ({
  id,
  title,
  children,
  className,
  ...
  const [dragHandles, setDragHandles] = useState<Element[]>([]);
  ...
  // Find and set draggable elements
  useEffect(() => {
    if (windowRef.current) {
      const draggableElements = windowRef.current.querySelectorAll('[data-draggable="true"]');
      setDragHandles(Array.from(draggableElements));
      
      // If no draggable elements found, use the header as default
      if (draggableElements.length === 0 && headerRef.current) {
        headerRef.current.setAttribute('data-draggable', 'true');
        setDragHandles([headerRef.current]);
      }
    }
  }, [windowRef.current, headerRef.current, theme, isMaximized]);
  ...
  return (
    ...
    >
      <motion.div
        ...
        onClick={(e) => {
          // Ensure window gets focus when clicked anywhere
          if (!isFocused) {
            onFocus();
          }
        }}
        ...
        animate={{ x, y }}
      >
        {/* Header with automatic data-draggable attribute */}
        <motion.div
          ref={headerRef}
          className={cn(
            "window-header",
            "window-drag-handle", // Add this class for consistency with older code
            activeTarget === "titlebar" && isActive && "active"
          )}
          data-draggable="true"
          onMouseDown={(e) => {
            // Make sure to focus the window when clicking the header
            onFocus();
            // Only prevent drag for control buttons
            if ((e.target as Element).closest(".window-controls")) {
              // clicking the controls → don't initiate a drag
              e.stopPropagation();
            }
          }}
          // Apply framer-motion drag only to the header if not maximized
          {...(isMaximized
            ? {}
            : {
                drag: true,
                dragMomentum: false,
                dragElastic: 0,
                dragControls: undefined, // Make sure we're not using external controls
                onDragStart: handleDragStart,
                onDrag: handleDrag,
                onDragEnd: handleDragEnd,
                dragTransition: { power: 0 },
                dragListener: true, // Ensure drag listener is enabled
                dragConstraints: false, // Don't constrain initially, we'll handle in onDragEnd
              })}
        >
          <WindowHeader
            title={title}
