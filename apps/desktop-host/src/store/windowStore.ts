  ...
  subscribeWithSelector(
    persist(
      (set, get) => ({
        ...
        // Computed properties - fixed for hydration
        get openWindows() {
          const state = get();
          if (!state || !state.windows) return [];
          return Object.values(state.windows).filter(window => window.isOpen);
        },

        get minimizedWindows() {
          const state = get();
          if (!state || !state.windows) return [];
          return Object.values(state.windows).filter(window => window.isMinimized);
        },

        get maximizedWindows() {
          const state = get();
          if (!state || !state.windows) return [];
          return Object.values(state.windows).filter(window => window.isMaximized);
        },

        // Optional initialization function
        init: () => {
          console.log('[WindowStore] Initialized');
        },
      }),
      {
