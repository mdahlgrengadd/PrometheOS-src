...
try {
  ...
  console.warn(
    'Could not load base URL from vite config, using default "/":',
    error.message
  );
}
...
async function generateDesktopShortcuts() {
  ...
  try {
    ...
    const manifestImports = registryContent.match(
      /import { manifest as \w+Manifest } from ["']\.\/apps\/([^"']+)\/manifest["']/g
    );
    ...
    const pluginIds = manifestImports
      .map((importLine) => {
    ...
    console.log(
      `Found ${pluginIds.length} plugins to create shortcuts for:`,
      pluginIds
    );
    ...
    for (const pluginId of pluginIds) {
      try {
        ...
        const manifestPath = path.resolve(
          __dirname,
          `../src/plugins/apps/${pluginId}/manifest.tsx`
        );
        ...
        try {
          ...
          console.log(
            `⚠ Could not read manifest for ${pluginId}, using defaults`
          );
        }
        ...
        console.error(
          `✗ Failed to create shortcut for ${pluginId}:`,
          error.message
        );
      }
    }
...
async function setupShadowEnvironment() {
  ...
  try {
    ...
    await generateDesktopShortcuts(); // 2d. Copy JSLinux assets to public directory
    console.log("Copying JSLinux assets to public directory...");
    await copyJSLinuxAssets();
...
/**
 * Copy JSLinux assets to the public directory
 */
async function copyJSLinuxAssets() {
  console.log("🚀 copyJSLinuxAssets function called");

  try {
    const jslinuxSrcDir = path.resolve(
      __dirname,
      "../src/plugins/apps/jslinux"
    );
    const jslinuxDestDir = path.resolve(__dirname, "../public/jslinux");

    // Ensure target directory exists
    await fs.mkdir(jslinuxDestDir, { recursive: true });

    // Copy all JSLinux files
    const files = await fg(["**/*"], {
      cwd: jslinuxSrcDir,
      dot: true,
      onlyFiles: true,
    ...
    for (const file of files) {
      ...
      const srcPath = path.join(jslinuxSrcDir, file);
      const destPath = path.join(jslinuxDestDir, file);
    ...
    console.log("✓ JSLinux assets copied successfully");
  } catch (error) {
    console.error("✗ Failed to copy JSLinux assets:", error);
  }
}
