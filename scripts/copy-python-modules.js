...
try {
  ...
  if (fs.existsSync(generatedSourceDir)) {
    ...
    if (fs.existsSync(generatedClientDir)) {
      const generatedTargetDir = path.join(
        targetDir,
        "prometheos_client_python_generated"
      );
    ...
    console.log(
      '⚠️  Generated client files not found. Run "npm run codegen" first.'
    );
  }

  console.log("🎉 Python modules copied successfully!");
  console.log(
    "💡 Pyodide can now fetch these modules from /prometheos/python-modules/"
  );
} catch (error) {
