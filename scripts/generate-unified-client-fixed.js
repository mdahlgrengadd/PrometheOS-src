...
// Legacy API for backward compatibility (DEPRECATED - use services instead)
export const launcher = {
  async launchApp(params: { appId: string }) {
    console.warn('DEPRECATED: Use services.launchApp() instead of launcher.launchApp()');
    return services.launchApp(params);
  },
  async killApp(params: { appId: string }) {
    console.warn('DEPRECATED: Use services.killApp() instead of launcher.killApp()');
    return services.killApp(params);
  },
  async notify(params: { message: string; type?: 'radix' | 'sonner' }) {
    console.warn('DEPRECATED: Use services.notify() instead of launcher.notify()');
    return services.notify(params);
  }
};
...
export const dialog = {
  async openDialog(params: { 
    ...
    console.warn('DEPRECATED: Use services.openDialog() instead of dialog.openDialog()');
    return services.openDialog(params);
  }
};

export const onEvent = {
  async waitForEvent(params: { eventId: string; timeout?: number }) {
    console.warn('DEPRECATED: Use services.waitForEvent() instead of onEvent.waitForEvent()');
    return services.waitForEvent(params);
  }
};

export const event = {
  async listEvents(params: Record<string, never> = {}) {
    console.warn('DEPRECATED: Use services.listEvents() instead of event.listEvents()');
    return services.listEvents(params);
  }
};
...
# Legacy API classes for backward compatibility (DEPRECATED - use Services instead)
class Launcher:
    """DEPRECATED: Use Services class instead. Launcher API for managing applications"""
    
    @staticmethod
    async def launch_app(app_id: str) -> Any:
        """DEPRECATED: Use Services.launch_app() instead"""
        print("WARNING: Launcher.launch_app() is deprecated. Use Services.launch_app() instead.")
        return await Services.launch_app(app_id)
    
    @staticmethod
    async def kill_app(app_id: str) -> Any:
        """DEPRECATED: Use Services.kill_app() instead"""
        print("WARNING: Launcher.kill_app() is deprecated. Use Services.kill_app() instead.")
        return await Services.kill_app(app_id)
    
    @staticmethod
    async def notify(message: str, notification_type: str = 'radix') -> Any:
        """DEPRECATED: Use Services.notify() instead"""
        print("WARNING: Launcher.notify() is deprecated. Use Services.notify() instead.")
        return await Services.notify(message, notification_type)


class Dialog:
    """DEPRECATED: Use Services class instead. Dialog API for user interactions"""
    ...
    async def open_dialog(
        ...
        """DEPRECATED: Use Services.open_dialog() instead"""
        print("WARNING: Dialog.open_dialog() is deprecated. Use Services.open_dialog() instead.")
        return await Services.open_dialog(title, description, confirm_label, cancel_label)


class OnEvent:
    """DEPRECATED: Use Services class instead. Event waiting API"""
    
    @staticmethod
    async def wait_for_event(event_id: str, timeout: Optional[int] = None) -> Any:
        """DEPRECATED: Use Services.wait_for_event() instead"""
        print("WARNING: OnEvent.wait_for_event() is deprecated. Use Services.wait_for_event() instead.")
        return await Services.wait_for_event(event_id, timeout)


class Event:
    """DEPRECATED: Use Services class instead. Event management API"""
    
    @staticmethod
    async def list_events() -> Any:
        """DEPRECATED: Use Services.list_events() instead"""
        print("WARNING: Event.list_events() is deprecated. Use Services.list_events() instead.")
...
import asyncio
from prometheos_client import services, launcher, dialog, on_event, event, api
...
async def example_legacy_usage():
    """Legacy API usage examples (deprecated but still functional)"""
    
    # These will show deprecation warnings
    try:
        result = await launcher.launch_app("audioplayer")
        print(f"Legacy launcher result: {result}")
    except Exception as e:
        print(f"Legacy launcher failed: {e}")
